...
- `go-build.yml`: push/PR마다 `amd64`, `arm/v7`, `arm64` 빌드 아티팩트를 생성합니다.
- `go-release.yml`: `v*` 태그 푸시 시 GitHub Release를 자동 생성(또는 기존 릴리즈에 추가 업로드)하고, 멀티아키 바이너리를 첨부합니다.
...
또는 GitHub Actions의 `workflow_dispatch`로 원하는 태그를 입력해 수동 릴리즈도 가능합니다.
